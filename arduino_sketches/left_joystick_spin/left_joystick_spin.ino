...
#define LEFT_JOYSTICK_Y_CHANNEL 2   // Channel 2 (Pin 10) is the left joystick Y-axis
...
void setup() {
  ...
  Serial.println("Starting ODriveCAN Left Joystick Spin Demo");
...
void loop() {
  ...
  // Get joystick input from left joystick (vertical axis)
  int joystickValue = receiver.getRaw(LEFT_JOYSTICK_Y_CHANNEL);
  ...
  // Fixed center values based on your calibration
  int centerValues[4] = {1500, 1495, 1480, 1500}; // Manually determined from your logs
  int joystickCenter = centerValues[LEFT_JOYSTICK_Y_CHANNEL-1];
  ...
  // Positive offset (joystick up) = positive velocity (clockwise spin)
  // Negative offset (joystick down) = negative velocity (counter-clockwise spin)
