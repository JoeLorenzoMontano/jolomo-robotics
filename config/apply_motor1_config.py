#!/usr/bin/env python3
"""
Apply Working ODrive S1 Configuration for MOTOR 1 (CAN Node ID 2)
Eaglepower 8308-90KV with 6S LiPo (6200mAh 120C)

This configuration is identical to Motor 0 except for CAN node ID.
Run this for the SECOND motor only.
"""
...
def apply_config():
    print("=" * 80)
    print("Applying Working ODrive S1 Configuration - MOTOR 1 (Node ID 2)")
    print("=" * 80)
    ...
    odrv.axis0.config.can.node_id = 2  # ← MOTOR 1 uses Node ID 2
    ...
    print("✓ CAN bus configured (Node ID 2)")
    ...
    try:
        ...
        print(f"Firmware: v{odrv.fw_version_major}.{odrv.fw_version_minor}.{odrv.fw_version_revision}")
        print(f"CAN Node ID: {odrv.axis0.config.can.node_id} (should be 2)")
        ...
        print("   python3 calibrate_motor1.py")
